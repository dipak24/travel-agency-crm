...
Route::middleware(['auth:customer', ResolveTenant::class])
    ...
    ->name('payments.')
    ->group(function (): void {
        Route::get('/{gateway}/{invoice}/start', [CheckoutController::class, 'start'])->name('checkout');
...
Route::prefix('pay')->name('public.pay.')->group(function (): void {
    ...
    Route::get('/{gateway}/{invoice}/start', [PublicPaymentController::class, 'start'])->name('start')->middleware('signed');
