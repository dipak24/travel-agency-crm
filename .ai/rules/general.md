...
## Every .env key leaks into tests via env_file — tests/bootstrap.php unsets all phpunit.xml keys
docker-compose.yml's `env_file: .env` makes EVERY .env key a real container env var, not just DB_*. So before 2026-09-25, all phpunit.xml overrides were ignored in the container: tests really ran with MAIL_MAILER=log, QUEUE_CONNECTION=database (dispatched jobs never ran), database cache/session, and APP_ENV=local. tests/bootstrap.php now reads phpunit.xml's own <env> names and unsets each from $_SERVER (plus DB_*). Add new test-only env overrides to phpunit.xml only; bootstrap picks them up automatically, so don't go back to a hand-kept key list. The full suite also needs `<ini name="memory_limit" value="512M"/>` in phpunit.xml: it outgrew 128 MB and fatals partway through.

## queue and scheduler containers only see new code after a rebuild
docker-compose.yml runs three app containers from the same image: `app` (Apache), `queue` (queue:work), and `scheduler` (schedule:work, added 2026-09-25). Source isn't bind-mounted, so `docker cp` into travel-agency-crm-app-1 only updates `app`. Queued jobs/notifications and scheduled commands keep running OLD code until `docker compose up -d --build`. A missing class there shows up as failed jobs, not as test failures. Rebuild before testing anything that runs through the queue or scheduler in the real environment. The image entrypoint runs `migrate --force` on every container start; `queue`/`scheduler` wait for `app` to be healthy, so migrations never run concurrently.
